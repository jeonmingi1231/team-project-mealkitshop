(() => {
    ...
    async function checkEmailDup() {
        ...
        try {
            // ★ 컨트롤러 매핑과 반드시 동일: /api/members/email/check
            const res = await fetch(`/api/members/email/check?email=${encodeURIComponent(email)}`, { method: 'GET' });
    ...
    form && form.addEventListener('submit', async (e) => {
        ...
        // ★ 여기서 실제 폼 제출 → /members/join (서버 저장) → redirect:/signup/success
        form.submit();
    });
})();
